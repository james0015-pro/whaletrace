...
// ============================================================
// ResonanceCard — 鯨魚共振訊號卡片
// 紫色邊框、專業彭博感、hover 展開機構明細
// ============================================================
    ...
    >
      ...
      <div className="mb-2">
        <span className="text-lg font-semibold text-text-primary tabular-nums">
          {formatCurrency(total_institutional_buy)}
        </span>
        <span className="text-xs text-text-tertiary ml-1">
          · {institution_count} 家機構
        </span>
      </div>
      ...
      <div className="mb-3 flex flex-wrap gap-x-3 gap-y-0.5">
        {institutions.map((inst) => (
          <span key={inst.name} className="text-[11px] text-text-tertiary">
            ...
            <span className="font-mono text-text-muted ml-1 tabular-nums">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          <div className="w-12 h-1.5 rounded-full bg-bg-elevated overflow-hidden">
            <div
              className="h-full rounded-full bg-signal-purple transition-all"
              style={{ width: `${signal_strength}%` }}
            />
          </div>
          <span className="text-xs font-mono font-bold text-signal-purple tabular-nums">
            {signal_strength}
          </span>
        </div>
      </div>
