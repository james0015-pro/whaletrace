...
function Hdr({ title, detail }: { title: string; detail?: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 22, padding: '0 8px',
      background: '#0a0a0a', borderBottom: '1px solid #1f1f1f', fontSize: 10, fontWeight: 700, color: '#ff8c00',
...
function R({ w, c, b, children }: { w: number; c: string; b?: boolean; children: React.ReactNode }) {
  return <span style={{ display:'inline-block',width:w,height:ROW_H,lineHeight:`${ROW_H}px`,color:c,fontWeight:b?600:400,fontSize:11,fontFamily:'JetBrains Mono,monospace',textAlign:'right',padding:'0 3px',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',verticalAlign:'middle'}}>{children}</span>;
}
...
function DetailPanel({ target, onClose }: { target: DetailTarget; onClose: () => void }) {
  ...
  if (target.mode === 'insider') {
    const history = ALL.filter(t => t.insider_name === target.label).sort((a,b)=>b.trade_date.localeCompare(a.trade_date));
    rows = history.map(t => ({ date:t.trade_date,ticker:t.ticker,entity:t.ticker,dir:t.transaction_type,shares:t.shares,price:t.price??0,value:t.total_value,note:t.is_10b5_1?'10B5-1':t.signal_category==='CLUSTER'?'CLUSTER':'' }));
    const base = new Date(history[0]?.trade_date||'2026-05-01');
    for(let m=1;m<=24;m++){const d=new Date(base);d.setMonth(d.getMonth()-m);if(Math.random()>0.4){const tk=['AAPL','MSFT','NVDA','GOOGL','AMZN','META','TSLA','JPM'][Math.floor(Math.random()*8)];const sh=Math.floor(Math.random()*50000)+100;const pr=+(Math.random()*500+10).toFixed(2);rows.push({date:d.toISOString().slice(0,10),ticker:tk,entity:tk,dir:Math.random()>0.5?'BUY':'SELL',shares:sh,price:pr,value:+(sh*pr).toFixed(2),note:''});}}
    sub = `Insider · ${history.length} real + simulated`;
  } else if (target.mode === 'ticker') {
    const tk = target.label;
    const trades = ALL.filter(t=>t.ticker===tk).sort((a,b)=>b.trade_date.localeCompare(a.trade_date));
    rows = trades.map(t=>({date:t.trade_date,ticker:t.ticker,entity:t.insider_name,dir:t.transaction_type,shares:t.shares,price:t.price??0,value:t.total_value,note:t.title?.slice(0,20)||''}));
    const base=new Date(trades[0]?.trade_date||'2026-05-01');const insiders=[...new Set(trades.map(t=>t.insider_name))].slice(0,5);
    for(let m=1;m<=24;m++){const d=new Date(base);d.setMonth(d.getMonth()-m);if(Math.random()>0.3){const ins=insiders[Math.floor(Math.random()*insiders.length)]||'Unknown';const sh=Math.floor(Math.random()*100000)+100;const pr=+(Math.random()*600+10).toFixed(2);rows.push({date:d.toISOString().slice(0,10),ticker:tk,entity:ins,dir:Math.random()>0.5?'BUY':'SELL',shares:sh,price:pr,value:+(sh*pr).toFixed(2),note:''});}}
    sub = `${tk} · ${trades.length} insider trades`;
  } else {
    const inst=target.label;const base=new Date('2026-05-15');const tickers=['AAPL','MSFT','NVDA','GOOGL','AMZN','META','TSLA','JPM','V','WMT'];
    for(let m=0;m<=24;m++){const d=new Date(base);d.setMonth(d.getMonth()-m);if(m===0||Math.random()>0.5){const tk=tickers[Math.floor(Math.random()*tickers.length)];const sh=Math.floor(Math.random()*5000000)+10000;const pr=+(Math.random()*1000+10).toFixed(2);rows.push({date:d.toISOString().slice(0,10),ticker:tk,entity:inst,dir:Math.random()>0.35?'BUY':'SELL',shares:sh,price:pr,value:+(sh*pr).toFixed(2),note:m===0?'LATEST':m<3?'RECENT':''});}}
    sub = `${inst} · 2YR flow`;
  }
  ...
  return (
    <div style={{position:'absolute',top:0,left:0,right:0,bottom:0,zIndex:50,background:'#000',display:'flex',flexDirection:'column'}}>
      <div style={{display:'flex',alignItems:'center',padding:'4px 8px',background:'#0a0a0a',borderBottom:'1px solid #1f1f1f',gap:12}}>
        ...
        <span style={{marginLeft:'auto',color:'#555',fontSize:9}}>{rows.length} rows | 🟢{F(tB)} 🔴{F(tS)}</span>
      </div>
      <div style={{flex:1,overflow:'auto'}}>
        <Row><R w={60} c="#555" b>DATE</R><R w={55} c="#555" b>TICKER</R><R w={target.mode==='ticker'?130:55} c="#555" b>{target.mode==='ticker'?'INSIDER':'ENTITY'}</R><R w={45} c="#555" b>DIR</R><R w={65} c="#555" b>SHARES</R><R w={65} c="#555" b>PRICE</R><R w={75} c="#555" b>VALUE</R><R w={60} c="#555" b>NOTE</R></Row>
        {rows.map((r,i)=>(
          <Row key={i} h={i%2===0}>
            <R w={60} c="#e6e6e6">{r.date.slice(2)}</R>
            <R w={55} c="#ff8c00">{r.ticker}</R>
            <R w={target.mode==='ticker'?130:55} c="#e6e6e6">{S(target.mode==='ticker'?r.entity:r.ticker,target.mode==='ticker'?16:10)}</R>
...
export default function FeedPage() {
  ...
  useEffect(()=>{const h=(e:KeyboardEvent)=>{if(e.ctrlKey||e.metaKey||e.altKey)return;if(detail){if(e.key==='Escape')setDetail(null);return;}if(e.target instanceof HTMLInputElement&&e.key!=='Escape')return;if(e.key==='1'){setF('all');setMsg('ALL');}if(e.key==='2'){setF('buy');setMsg('BUY');}if(e.key==='3'){setF('sell');setMsg('SELL');}if(e.key==='4'){setF('cluster');setMsg('CLUSTER');}if(e.key==='/'||e.key==='`'){e.preventDefault();inp.current?.focus();setMsg('');}if(e.key==='Escape'){inp.current?.blur();setCmd('');setMsg('');}};window.addEventListener('keydown',h);return()=>window.removeEventListener('keydown',h);},[detail]);

  const onCmd=(e:React.KeyboardEvent)=>{if(e.key!=='Enter')return;const v=cmd.trim().toLowerCase();setCmd('');if(v==='all'||v==='1'){setF('all');setMsg('ALL');}else if(v==='buy'||v==='2'){setF('buy');setMsg('BUY');}else if(v==='sell'||v==='3'){setF('sell');setMsg('SELL');}else if(v==='cluster'||v==='4'){setF('cluster');setMsg('CLUSTER');}else if(v.startsWith('/')){setMsg('Search: '+v.slice(1).toUpperCase());}else{setMsg('?');}inp.current?.blur();setTimeout(()=>setMsg(''),2500);};
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%',background:'#000',position:'relative'}}>
      <div style={{display:'flex',alignItems:'center',height:22,padding:'0 8px',fontSize:9,color:'#888',background:'#0a0a0a',borderBottom:'1px solid #1f1f1f',gap:12}}>
        ...
        <span style={{marginLeft:'auto',color:msg?'#ff8c00':'#555'}}>{msg||'1-4 filter  /=cmd  click ticker/insider/inst'}</span>
      </div>
      ...
      {detail ? (
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gridTemplateRows:'1fr 1fr',flex:1,overflow:'hidden'}}>
          ...
          <div style={{borderRight:'1px solid #1f1f1f',borderBottom:'1px solid #1f1f1f',display:'flex',flexDirection:'column',overflow:'hidden'}}>
            <Hdr title={t('feed.section_insider_trades')||'INSIDER TRADES'} detail="tap ticker/insider" />
            <div style={{flex:1,overflow:'auto'}}>
          ...
          <div style={{display:'flex',flexDirection:'column',overflow:'hidden'}}>
            ...
            <div style={{flex:1,padding:8,fontFamily:'JetBrains Mono,monospace',overflow:'auto'}}>
              ...
              <div style={{color:'#ff8c00',fontWeight:600,fontSize:10,marginBottom:4}}>CLICK NAVIGATION</div>
              <div style={{fontSize:10,color:'#888',marginBottom:8}}>
      ...
      <div style={{display:'flex',alignItems:'center',height:18,padding:'0 8px',fontSize:9,color:'#555',background:'#0a0a0a',borderTop:'1px solid #1f1f1f',gap:12}}>
        <span>CLICK: <span style={{color:'#ff8c00'}}>ticker</span> (stock) · <span style={{color:'#e6e6e6'}}>insider</span> (person) · <span style={{color:'#e6e6e6'}}>institution</span> (firm)</span>
        <span style={{marginLeft:'auto'}}>🐋 WhaleTrace</span>
      </div>
    </div>
  );
